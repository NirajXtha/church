...
function createPresentationWindow() {
  ...
  presentationWin.loadFile(
    path.join(__dirname, "renderer", "presentation.html"),
  );
...
function setupIPC() {
  ...
  ipcMain.handle("get-chapter-verses", (_, bookId, chapter, lang) =>
    db.getChapterVerses(bookId, chapter, lang),
  );
  ipcMain.handle("get-chapter-verses-dual", (_, bookId, chapter) =>
    db.getChapterVersesDual(bookId, chapter),
  );
  ipcMain.handle(
    "get-verses-dual",
    (_, bookId, chapter, startVerse, endVerse) =>
      db.getVersesDual(bookId, chapter, startVerse, endVerse),
  );
  ipcMain.handle("search-verses-dual", (_, keyword) =>
    db.searchVersesDual(keyword),
  );
  ipcMain.handle("get-all-songs", () => db.getAllSongs());
  ipcMain.handle("get-song-lyrics", (_, id) => db.getSongLyrics(id));
  ipcMain.handle(
    "add-song",
    (_, title, lyrics, category, language, author, tags) =>
