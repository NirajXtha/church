...
function getBooks(lang) {
  ...
  return d
    .prepare(
      "SELECT id, name_nepali, name_english, testament FROM books ORDER BY id",
    )
    .all();
}
...
function searchVerses(keyword, lang = "nepali") {
  ...
  return d
    .prepare(
      `SELECT v.verse, v.text, v.chapter, v.book, b.name_nepali as book_nepali, b.name_english as book_english FROM ${table} v JOIN books b ON v.book = b.id WHERE v.text LIKE ? LIMIT 50`,
    )
    .all(`%${keyword}%`);
}
...
function getBookNames() {
  const d = getDB();
  return d
    .prepare("SELECT id, name_nepali, name_english FROM books ORDER BY id")
    .all();
}
...
function searchVersesDual(keyword) {
  ...
  const results = d
    .prepare(
      `SELECT v.verse, v.text, v.chapter, v.book, b.name_nepali as book_nepali, b.name_english as book_english FROM verses v JOIN books b ON v.book = b.id WHERE v.text LIKE ? LIMIT 30`,
    )
    .all(`%${keyword}%`);
  const enResults = d
    .prepare(
      `SELECT v.verse, v.text, v.chapter, v.book, b.name_nepali as book_nepali, b.name_english as book_english FROM en_verses v JOIN books b ON v.book = b.id WHERE v.text LIKE ? LIMIT 30`,
    )
    .all(`%${keyword}%`);
